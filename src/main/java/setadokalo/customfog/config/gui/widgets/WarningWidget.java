...
public class WarningWidget implements Drawable, Element, Selectable {
	...
	public static void drawNinepatchRect(
		...
		context.drawTexture(null, x + width - edgeSize, y,
		...
		context.drawTexture(null, x + width - edgeSize, y + height - edgeSize,
	...
	public void render(DrawContext context, int mouseX, int mouseY, float delta) {
		...
		context.getMatrices().translate( 0.0, 0.0, 2.5);
		RenderSystem.enableDepthTest();
		RenderSystem.setShaderTexture(0, new Identifier("custom-fog", "textures/gui/cfog-gui.png"));
		// Background
		drawNinepatchRect(context, this.x, this.y,
				width, height,
				0, 0,
				5, 10);
		// Type icon
		context.drawTexture(null, this.x + 5, this.y + (this.height - 20)/2,
...
//			textRenderer.drawWithShadow(matrices, orderedText, this.x + 20, this.y + y, color);
			context.drawCenteredTextWithShadow(textRenderer, orderedText, this.x + 10 + width / 2, this.y + ty, color);
			ty += textRenderer.fontHeight + 2;
		}
		if (closeBtn != null) {
			closeBtn.setX(x + width - 8);
			closeBtn.setY(y);
			closeBtn.render(context, mouseX, mouseY, delta);
		}
